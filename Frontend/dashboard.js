...
function renderTable(logs) {
    ...
    tbody.innerHTML = logs.map((log, index) => {
        ...
        const isMarked = window.markedFalsePositives && 
                        window.markedFalsePositives.some(fp => 
                            fp.url === log.url && fp.timestamp === log.timestamp
                        );
            ...
            <td colspan="7">
                <div class="details-panel">
                    <div class="details-grid">
                        ...
                        <div class="detail-item">
                            <strong>🌐 Full URL:</strong> <span style="word-break: break-all;">${log.url}</span>
                        </div>
                    </div>
...
function toggleDetails(index) {
    ...
    if (detailsRow.style.display === 'none') {
        ...
        document.querySelectorAll('.data-table tbody tr:not(.details-row)').forEach(row => {
...
async function confirmFalsePositive() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Refresh stats and table to show "Reviewed" badge
            fetchStats();
...
applyFilters = function() {
    ...
    if (window.customFilterType && window.customFilterValue) {
        filtered = filterByCustomSelection(filtered, window.customFilterType, window.customFilterValue);
    }
